// State
const DATA_URL = 'data/offices.json';
let allData = [];
let filteredData = [];
let currentPage = 1;
...
document.addEventListener('DOMContentLoaded', async () => {
    // Auth check skipped for simplicity in this demo
    await loadData();
    setupEvents();
});
...
async function loadData() {
    try {
        ...
        console.error('Error loading offices data', err);
    }
}
...
function renderTable() {
    ...
    tableBody.innerHTML = pageData.map((item, index) => `
        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
            ...
            <td class="p-4 border-b border-gray-100 dark:border-slate-700 text-sm text-gray-600 dark:text-gray-300">${item.governorate || '-'}</td>
            <td class="p-4 border-b border-gray-100 dark:border-slate-700 text-sm font-mono text-gray-500">${item.phone || '-'}</td>
            <td class="p-4 border-b border-gray-100 dark:border-slate-700 text-sm text-gray-600 dark:text-gray-300">${item.manager || '-'}</td>
...
function filterData() {
    ...
    filteredData = allData.filter(item =>
        ...
        (item.governorate && item.governorate.toLowerCase().includes(term)) ||
...
window.editItem = (index) => {
    ...
    document.getElementById('inpGov').value = item.governorate || '';
    document.getElementById('inpAddress').value = item.address || '';
    document.getElementById('inpPhone').value = item.phone || '';
    document.getElementById('inpManager').value = item.manager || '';
    document.getElementById('inpType').value = item.type || 'فرع';
...
itemForm.addEventListener('submit', (e) => {
    ...
    const newItem = {
        ...
        governorate: document.getElementById('inpGov').value,
